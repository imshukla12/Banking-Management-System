...
void addjointAccount(char uname1[], char pass1[], char uname2[], char pass2[], int sd)
{
	...
	write(sd, "Account Created", sizeof("Account Created"));
...
void newJointAccount(int sd, int flag)
{
	...
	{
		write(sd, "Username already exist", sizeof("Username already exist"));
		if(flag ==1)
			mainMenu(sd);
		else
			admin_functions(sd);
...
void login(int sd)
{
	...
	while(read(singlefd, (char *)&db, sizeof(struct database)))
	{
		...
		if(!strcmp(db.uname, uname) && !strcmp(db.pass, pass))
			...
			// This if call is used to avoid login of same customer again and again..
				...
				write(sd, "Login Successful", sizeof("Login Successful"));
				displayMenu(sd,db);
			        close(singlefd);
				return;		
			}
	...
	flag=0;
	write(sd, &flag, sizeof(flag));
	if(f!=1) write(sd, "Invalid Credentials", sizeof("Invalid Credentials"));
	else write(sd, "User Already logged in, Try again later", sizeof("User Already logged in, Try again later"));
	mainMenu(sd);	
}
...
void jointAccountLogin(int sd,int fd)
{
	...
	write(sd, &flag, sizeof(flag));
	if(f!=1) write(sd, "Invalid Credentials", sizeof("Invalid Credentials"));
	else write(sd, "Another user logged in", sizeof("Another user logged in"));
	mainMenu(sd);
}
...
void admin_functions(int sd)
{
	...
	switch(choice)
	{
		...
		case 3:read(sd, uname, sizeof(uname));
				if(checkAccountExists(uname))
			{
				write(sd, "Account Found", sizeof("Account Found"));
			}
			else
			{
				write(sd, "Account does not exist with the given Username", sizeof("Account does not exist with the given Username"));
			}
			admin_functions(sd);
			break;
...
void deleteAccount(int sd)
{
	...
	if(!checkAccountExists(uname))
	{
		write(sd, "Account does not exist", sizeof("Account does not exist"));
	}
	else
	{	
...
void adminLogin(int sd)
{
	...
	read(sd, uname, sizeof(uname));
	read(sd, pass, sizeof(pass));
	if(!strcmp(uname,"ss") && !strcmp(pass,"123456789"))
	{
		write(sd, &flag, sizeof(flag));
		write(sd,"Welcome Admin", sizeof("Welcome Admin"));
		admin_functions(sd);
	}
	else
	{	
		flag=0;
		write(sd, &flag, sizeof(flag));
		write(sd,"Invalid Credentials", sizeof("Invalid Credentials"));
		mainMenu(sd);
	}
